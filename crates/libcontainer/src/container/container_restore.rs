...
use super::container_criu::{handle_restoring_namespaces, CRIU_VERSION_MINIMUM};
...
fn restore_callback(script: &str, notify: &Criu_notify, fds: &[RawFd]) -> i32 {
    match script {
        ...
        "orphan-pts-master" => {
            ...
            tracing::debug!(master_fd, "received orphan-pts-master notification from CRIU");
            RESTORE_CTX.with(|ctx| {
                let borrow = ctx.borrow();
                match &*borrow {
                    Some(context) => {
                        if let Some(ref console_socket) = context.console_socket {
                            tracing::debug!(?console_socket, master_fd, "sending PTY master to console socket");
...
impl Container {
    ...
    fn get_rootfs_and_spec(
        ...
        let root = spec.root().as_ref().ok_or_else(|| {
            LibcontainerError::Other("spec has no root".to_string())
        })?;
