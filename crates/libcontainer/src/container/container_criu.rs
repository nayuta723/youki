...
impl Container {
    ...
    pub fn check_criu_version(&mut self, min_version: u32) -> Result<(), LibcontainerError> {
        ...
        let version = criu.get_criu_version().map_err(|e| {
            LibcontainerError::Other(format!("CRIU version check failed: {}", e))
        })?;
...
mod tests {
    ...
    fn test_criu_ns_to_key() {
        ...
        assert_eq!(criu_ns_to_key(LinuxNamespaceType::Cgroup), "extRootCgroupNS");
    }
}
